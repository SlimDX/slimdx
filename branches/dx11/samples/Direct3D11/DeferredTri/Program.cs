...
namespace DeferredTri
{
    ...
    static class Program
    {
        ...
        static D3D11.Device Device;
        static D3D11.InputLayout Layout;
        static D3D11.Buffer Vertices;
        static D3D11.RenderTargetView RenderTarget;
        static D3D11.VertexShader VertexShader;
        static D3D11.PixelShader PixelShader;
        static D3D11.DeviceContext DeferredContext;
        static D3D11.CommandList CommandList;
        ...
        static void Main()
        {
            ...
            RenderForm.Text = "SlimDX - DeferredTri Direct3D 11 Sample";
            ...
            D3D11.Device.CreateWithSwapChain(null, D3D11.DriverType.Hardware, D3D11.DeviceCreationFlags.Debug, swapChainDescription, out Device, out SwapChain);


            using (D3D11.Texture2D resource = SwapChain.GetBuffer<D3D11.Texture2D>(0))
            {
            ...
            D3D11.ShaderBlob blob;
            string errors;
            VertexShader = D3D11.VertexShader.CompileFromFile(Device, "MiniTri.fx", "VS", "vs_4_0", out blob, out errors);
            PixelShader = D3D11.PixelShader.CompileFromFile(Device, "MiniTri.fx", "PS", "ps_4_0", out errors);

            Layout = new D3D11.InputLayout(Device, inputElements, blob);
            ...
            bufferDescription.Usage = D3D11.ResourceUsage.Default;

            Vertices = new D3D11.Buffer(Device, stream, bufferDescription);
            stream.Close();

            DeferredContext = Device.CreateDeferredContext();
            DeferredContext.ClearState();
            DeferredContext.Rasterizer.SetViewports(ViewArea);
            DeferredContext.OutputMerger.SetTargets(RenderTarget);

            DeferredContext.ClearRenderTargetView(RenderTarget, Color.Black);

            DeferredContext.InputAssembler.SetInputLayout(Layout);
            DeferredContext.InputAssembler.SetPrimitiveTopology(D3D11.PrimitiveTopology.TriangleList);
            DeferredContext.InputAssembler.SetVertexBuffers(0, new SlimDX.Direct3D11.VertexBufferBinding(Vertices, Marshal.SizeOf(typeof(Vertex)), 0));
            ...
            CommandList = DeferredContext.FinishCommandList(true);
