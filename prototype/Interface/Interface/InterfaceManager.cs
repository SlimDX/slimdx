...
namespace Interface
{
    ...
    public class InterfaceManager
    {
        ...
        public void SetTexture(string textureFileName)
        {
            ...
            textureSize = new Size(info.Width, info.Height);
        }
        ...
        /// <summary>
        /// Called when the Direct3D device is created.
        /// </summary>
        ...
        /// <summary>
        /// Called when the Direct3D device is destroyed.
        /// </summary>
        ...
        public void Render()
        {
            ...
            Dialog[] visibleDialogs = Dialogs.Where(d => d.IsVisible).ToArray();
        ...
        void LoadXml(XDocument document)
        {
            ...
            XElement root = document.Element("SlimUI");
            if (root == null)
                throw new InvalidDataException("Invalid XML layout document (requires a SlimUI root element)");

            try
            {
                // check to see if we have a GlobalLayout element
                XElement layout = root.Element("GlobalLayout");
                if (layout != null)
            ...
            // check to see if we have an Interface element
            XElement interfaceElement = root.Element("Interface");
        ...
        void ProcessInterface(XElement interfaceElement)
        {
            // check for all dialog elements
            IEnumerable<XElement> dialogs = interfaceElement.Elements("Dialog");
            foreach (XElement dialogElement in dialogs)
            {
                // create the dialog
                ...
                IEnumerable<XAttribute> attributes = dialogElement.Attributes();
                foreach (XAttribute attribute in attributes)
                {
                    // search for a property of the given name
                    PropertyInfo info = dialogType.GetProperty(attribute.Name.LocalName);
                    if (info == null)
                ...
                // load all controls
                IEnumerable<XElement> controlElements = dialogElement.Elements();
                foreach (XElement controlElement in controlElements)
                {
                    // grab the name of the control (full name is optional)
                    string name;
                    XAttribute fullName = controlElement.Attribute("FullName");
                    if (fullName != null)
                        name = fullName.Value;
                    else
                        name = "Interface." + controlElement.Name.LocalName;
                    ...
                    IEnumerable<XAttribute> controlAttributes = controlElement.Attributes();
                    foreach (XAttribute attribute in controlAttributes)
                    {
                        // skip the FullName property
                        if (attribute.Name.LocalName == "FullName")
                            continue;

                        // search for a property of the given name
                        PropertyInfo info = controlType.GetProperty(attribute.Name.LocalName);
                        if (info == null)
        ...
        void ProcessLayout(XElement layout)
        {
            // process each child element
            IEnumerable<XElement> elements = layout.Elements();
            foreach (XElement element in elements)
            {
                // grab the name of the control (full name is optional)
                string name;
                XAttribute fullName = element.Attribute("FullName");
                if (fullName != null)
                    name = fullName.Value;
                else
                    name = "Interface." + element.Name.LocalName;

                // process each element
                IEnumerable<XElement> drawingElements = element.Elements("Element");
                List<DrawingElement> actualElements = new List<DrawingElement>();
                foreach (XElement drawingElement in drawingElements)
        ...
        DrawingElement ProcessElement(XElement element)
        {
            ...
            XElement textureRectangleElement = element.Element("TextureRectangle");
            if (textureRectangleElement != null)
                drawingElement.TextureRectangle = textureRectangleElement.Value.ToObject<Rectangle>();

            // check for a text format
            XElement textFormat = element.Element("TextFormat");
            if (textFormat != null)
            {
                try
                {
        ...
        BlendColor ReadBlendColor(XElement parent, string name)
        {
            // read the element
            ...
            XAttribute attribute = element.Attribute("Default");
            if (attribute != null)
            {
                // create the object using the default value
                Color4 defaultColor = attribute.Value.ToObject<Color4>();
                result = new BlendColor(defaultColor);
            }

            // read each state specifier
            IEnumerable<XElement> stateElements = element.Elements("State");
            foreach (XElement stateElement in stateElements)
            {
                // grab the name (NOT optional)
                XAttribute stateNameAttribute = stateElement.Attribute("Name");
                ...
                Color4 value = stateElement.Value.ToObject<Color4>();
